...
void Renderer::render_parts()
{
	...
	for (i = 0; i <= sim->parts_lastActiveIndex; i++) {
		if (sim->parts[i].type && sim->parts[i].type >= 0 && sim->parts[i].type < PT_NUM) {
			...
			if (sim->drawWires)
				for (int nodeCount = 0; nodeCount < sim->parts[i].inputs.size(); nodeCount++) {
					WireIn node = sim->parts[i].inputs[nodeCount];
					if (node.connI != -1) {
						int ox = sim->parts[i].x;
						int oy = sim->parts[i].y;
						int dx = ox + node.connX;
						int dy = oy + node.connY;

						draw_line(ox, oy, dx, dy, 128, 255, 128, 32);

						int c1 = sim->elements[t].Colour;
						int r1 = (c1 >> 16) & 0xFF;
						int g1 = (c1 >> 8) & 0xFF;
						int b1 = c1 & 0xFF;

						int c2 = sim->elements[TYP(sim->pmap[dy][dx])].Colour;
						int r2 = (c2 >> 16) & 0xFF;
						int g2 = (c2 >> 8) & 0xFF;
						int b2 = c2 & 0xFF;

						blendpixel(ox, oy, r1, g1, b1, 255);
						blendpixel(dx, dy, r2, g2, b2, 255);
					}
				}
