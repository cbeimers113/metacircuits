...
void Client::DeleteStamp(ByteString stampID)
{
	for (std::list<ByteString>::iterator iterator = stampIDs.begin(), end = stampIDs.end(); iterator != end; ++iterator)
	{
		if ((*iterator) == stampID)
		{
			...
			ByteString circuitFilename = ByteString::Build(CIRCUITS_DIR, PATH_SEP, stampID, ".cct");
			remove(stampFilename.c_str());
			remove(circuitFilename.c_str());
			stampIDs.erase(iterator);
			break;
		}
	}
...
ByteString Client::AddStamp(GameSave* saveData)
{
	...
	ByteString circuitName = CIRCUITS_DIR PATH_SEP + saveID + ".cct";

	MakeDirectory(STAMPS_DIR);
	MakeDirectory(CIRCUITS_DIR);

	//Save circuit connections separately
	if (saveData->HasCircuit()) {
		std::ofstream circuitStream;
		circuitStream.open(circuitName.c_str());
		for (int cct = 0; cct < saveData->particlesCount; cct++)
			if (saveData->particles[cct].inputs.size() || saveData->particles[cct].outputs.size()) {
				Particle cp = saveData->particles[cct];
				circuitStream << cp.x << "." << cp.y << "{\n";

				for (int cNode = 0; cNode < cp.inputs.size(); cNode++) {
					WireIn node = cp.inputs[cNode];
					circuitStream << "i." << node.name.ToAscii() << "." << node.connX << "." << node.connY << "." << node.connT << "." << node.connI << ";\n";
				}

				for (int cNode = 0; cNode < cp.outputs.size(); cNode++) {
					String node = cp.outputs[cNode];
					circuitStream << "o." << node.ToAscii() << ";\n";
				}
