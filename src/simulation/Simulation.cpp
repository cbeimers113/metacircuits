...
int Simulation::LoadCircuit(ByteString id, int x, int y) {
	//Align to blockMap
	int blockX = (x + CELL / 2) / CELL;
	int blockY = (y + CELL / 2) / CELL;
	x = blockX * CELL;
	y = blockY * CELL;

	ByteString fname = ByteString(CIRCUITS_DIR PATH_SEP + id + ".cct");
	std::ifstream cData(fname);
	if (cData) {
		std::string line;
		bool wiring = false;
		bool posData = false;
		...
		while (std::getline(cData, line)) {
			...
			if (wiring) {
				std::istringstream data(line.substr(0, line.length() - 1));
				...
				if (posData) {
					// Extract position 
					xOffs = std::stoi(tokens[0]);
					yOffs = std::stoi(tokens[1]);
					posData = false;
				}
				else {
					// Extract connection data
					String name = std_string_to_String(tokens[1]);
					if (tokens[0] == "i")
						parts[ID(pmap[y + yOffs][x + xOffs])].inputs.push_back(WireIn{ name, std::stoi(tokens[2]), std::stoi(tokens[3]), std::stoi(tokens[4]), std::stoi(tokens[5]) });
					else if (tokens[0] == "o")
						parts[ID(pmap[y + yOffs][x + xOffs])].outputs.push_back(name);
				}
			}
		}
	}
...
void Simulation::UpdateParticles(int start, int end)
{
	...
	//First update circuit outputs, then everything else
	for (int updTypes = 0; updTypes < 2; updTypes++)
		for (i = start; i <= end && i <= parts_lastActiveIndex; i++)
			if (parts[i].type && (updTypes ^ (parts[i].type == PT_DIOD || parts[i].type == PT_LED || parts[i].type == PT_TRNS)))
			{
				t = parts[i].type;
				...
				// Remove wire connections from components that have been destroyed
				for (int nodeCount = 0; nodeCount < parts[i].inputs.size(); nodeCount++) {
					WireIn* node = &(parts[i].inputs[nodeCount]);
					if (node->connI == -1)
						continue;

					int dx = parts[i].x + node->connX;
					int dy = parts[i].y + node->connY;
					if (!pmap[dy][dx] || TYP(pmap[dy][dx]) != node->connT) {
						node->connX = 0;
						node->connY = 0;
						node->connT = 0;
						node->connI = -1;
