...
void GameController::PlaceSave(ui::Point position)
{
	...
	if (placeSave)
	{
		...
		{
			if (placeSave->HasCircuit())
				if (gameModel->GetSimulation()->LoadCircuit(GetCircuitID(), position.X, position.Y)) {
					//Handle couldn't load circuit error
				}
...
void GameController::LoadCircuitID(ByteString id) {
	gameModel->SetCircuitID(id);
}

ByteString GameController::GetCircuitID() {
	return gameModel->GetCircuitID();
}
...
void GameController::WireToolClick(int wireToolIndex, ui::Point fromPos, ui::Point toPos) {
	Simulation* sim = gameModel->GetSimulation();
	WireTool* wireTool = static_cast<WireTool*>(gameModel->GetActiveTool(wireToolIndex));
	int fromID = sim->pmap[fromPos.Y][fromPos.X];
	int toID = sim->pmap[toPos.Y][toPos.X];
	Particle* fromPart = fromID ? &(sim->parts[ID(fromID)]) : NULL;
	Particle* toPart = toID ? &(sim->parts[ID(toID)]) : NULL;
	wireTool->Connect(sim, fromPart, toPart);
}
...
void GameController::ToggleDrawWires()
{
	gameModel->SetDrawWiresEnable(!gameModel->GetDrawWiresEnable());
}
...
void GameController::OpenStamps()
{
	localBrowser = new LocalBrowserController([this] {
		...
		if (file)
		{
			...
			std::string trimmed = fullname.substr(first, last - first);

			LoadCircuitID(ByteString(trimmed));
