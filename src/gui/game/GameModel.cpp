...
void GameModel::BuildQuickOptionMenu(GameController * controller)
{
	...
	quickOptions.push_back(new DrawWiresOption(this));
...
void GameModel::BuildMenus()
{
	...
	menuList[SC_CRACKER]->AddTool(new WireTool(this));
	menuList[SC_CRACKER]->AddTool(new UnplugTool(this));
	menuList[SC_CRACKER]->AddTool(new WireTracer(this));
...
void GameModel::SetDrawWiresEnable(bool drawWires)
{
	sim->drawWires = drawWires;
	UpdateQuickOptions();
	if (drawWires)
		SetInfoTip("Drawing Circuit Wires");
	else
		SetInfoTip("Not Drawing Circuit Wires");
}

bool GameModel::GetDrawWiresEnable()
{
	return sim->drawWires;
}
...
void GameModel::SetCircuitID(ByteString id) {
	circuitID = id;
}

ByteString GameModel::GetCircuitID() {
	return circuitID;
}
